...
export class EventManagerComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        this.sub = this.router.events.subscribe(async val => {
            if (val instanceof NavigationEnd) {
                ...
                if (this.events_id) {
                    let event: any = await this.expanseService.start().then(() => this.expanseService.getEvent(this.events_id));
                    let date = new Date(+event.start_time * 1000);
                    date.setHours(0, 0, 0, 0);
                    this.selectedTime = (+event.start_time - Math.floor(date.getTime() / 1000)) / 60;
                    this.selectedDate = {
                        start: moment(date),
                        end: null,
                    };
                    ...
                    if (this.currentApp.apps_id) {
                        await this.expanseService.getApp(this.currentApp.apps_id).then(app => (this.selectedApp = app[0]));
                    }
    ...
    refreshShareLink() {
        return fetch('https://sdq.st/delete-link/' + this.expanseService.currentSession.token + '/e-' + this.events_id, {
            method: 'GET',
            cache: 'no-cache',
        })
            .then(() =>
                fetch('https://sdq.st/get-link/' + this.expanseService.currentSession.token, {
                    ...
                    body: JSON.stringify({
                        title: (this.currentApp.event_name || this.currentApp.name) + 'on Sidequest',
                        description: this.currentApp.event_description || this.currentApp.description,
                        image: this.currentApp.event_image || this.currentApp.image,
                        name: 'e-' + this.events_id,
                        external: this.eventUrl(),
                    }),
                })
            )
            .then(r => r.json())
    ...
    private eventUrl() {
        ...
        const path = this.router.createUrlTree(['/event', this.events_id]);
    ...
    saveEvent() {
        ...
        const start_time = Math.floor(dateSelected.getTime() / 1000) + this.selectedTime * 60;
        if (this.events_id) {
