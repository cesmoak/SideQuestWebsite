...
export class UserProfileComponent implements OnInit, OnDestroy {
            ...
            return this.expanseService
                ...
                .then((res: any) => {
                    if (res.length) {
                        ...
                        if (this.appService.isAuthenticated && this.expanseService.currentSession.users_id !== this.users_id) {
                ...
                .then(app_urls => {
                    ...
                    this.donate_urls = app_urls.filter(
                        (url: AppUrl) => ['Patreon', 'Paypal', 'Itch', 'Kofi'].indexOf(url.provider) > -1
                    );
                    ...
                    this.website_url = app_urls.filter((url: AppUrl) => url.provider === 'Website');
                })
                ...
                .then(() => {
                    if (this.appService.isAuthenticated && this.expanseService.currentSession.users_id !== this.users_id) {
                        return this.expanseService.getRelated(this.users_id || this.add_users_id).then(related => {
    ...
    setProfileColor() {
        this.expanseService.setProfileColor(this.users_id, this.isProfileColor ? 'golden' : '');
    }

    getPopularApps() {
        return this.expanseService
            .start()
            .then(() => this.expanseService.searchApps('', 0, 'rating', 'desc', null, null, this.users_id, 6))
    ...
    confirmActionGo() {
        ...
        switch (this.actionString) {
            case 'add this sidekick':
                promise = this.expanseService.addFriend(this.users_id || this.add_users_id);
                break;
            case 'accept this sidekick request':
                promise = this.expanseService.acceptFriendRequest(this.users_id || this.add_users_id);
                break;
            case 'cancel this sidekick request':
            case 'reject this sidekick request':
                promise = this.expanseService.rejectFriendRequest(this.users_id || this.add_users_id);
                break;
            case 'remove this sidekick':
                promise = this.expanseService.removeFriend(this.users_id || this.add_users_id);
                break;
            case 'block this sidekick':
                console.log('blocking');
                promise = this.expanseService.blockUser(this.users_id || this.add_users_id, 1);
                break;
            case 'unblock this sidekick':
                promise = this.expanseService.unBlockUser(this.users_id || this.add_users_id);
                break;
            case 'report this sidekick':
                promise = this.expanseService
                    .reportUser(this.users_id || this.add_users_id, this.reportType, this.reportDetails)
                    .then((res: any) => {
                        if (res.length) {
                            return fetch(this.expanseService.discordURl + '/new_report/' + res[0].user_reports_id);
                        }
                    });
                this.reportDetails = '';
                this.reportType = '1';
        ...
        return promise
            .then(res => {
                this.isLoading = true;
                return this.expanseService.getRelated(this.users_id || this.add_users_id);
            })
            .then(related => {
    ...
    getApps() {
        ...
        return this.expanseService
            .start()
            .then(() => this.expanseService.searchApps(this.searchString, this.page, 'name', 'desc', null, null, this.users_id))
    ...
    getSpaces() {
        this.isLoading = true;
        this.hasNoMore = false;
        return this.expanseService.getSpaces(this.page, this.searchString, this.users_id).then((res: any) => {
    ...
    getEvents(type?) {
        ...
        this.hasNoMore = false;
        return this.expanseService
            .getEvents(this.page, this.searchString, type || this.eventsType, this.users_id)
            .then((res: any) => {
                this.hasNoMore = !res.length;
                this.isLoading = false;
    ...
    public appUrl(app: AppListing) {
        return this.router.createUrlTree(['/app', app.apps_id, this.appService.appParamName(app)]);
    }
}
